...
hero_image: /images/hero-kubernetes-rolling-update.png
comments: true
---
...
As you might have guessed from the `MY_APPLICATION_VERSION ` syntax, the above is a jinja2 template. The `MY_APPLICATION_VERSION` variable's value is supplied at deploy time (from the deployment script, pipeline etc.) The RC template is usually saved as `my-application-rc.yml.tpl`
